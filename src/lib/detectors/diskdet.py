...
from models.decode import diskdet_decode
from models.utils import flip_tensor
from utils.post_process import diskdet_post_process
from .base_detector import BaseDetector
import cv2
from utils.image import get_affine_transform, affine_transform

class DiskdetDetector(BaseDetector):
  def __init__(self, opt):
    super(DiskdetDetector, self).__init__(opt)
  ...
  def process(self, images, return_time=False):
    with torch.no_grad():
      ...
      dets = diskdet_decode(hm, polys, pseudo_depth, reg=reg, cat_spec_poly=self.opt.cat_spec_poly, K=self.opt.K, rep = self.opt.rep)
    if return_time:
