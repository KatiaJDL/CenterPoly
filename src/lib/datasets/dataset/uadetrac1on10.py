...
class UADETRAC1ON10(data.Dataset):
    num_classes = 4
    default_resolution = [512, 896]  # [960, 540]
    ...
    def __init__(self, opt, split):
        ...
        if split == 'test':
            ...
            self.annot_path = '/Store/datasets/UA-Detrac/COCO-format/val-1-on-10.json'
        else:
            self.annot_path = '/Store/datasets/UA-Detrac/COCO-format/train-1-on-10.json'

        self.max_objs = 50  # 128
        self.class_name = [
        ...
        for image_id in all_bboxes:
            for cls_ind in all_bboxes[image_id]:
                ...
                for index in range(all_bboxes[image_id][cls_ind].shape[0]):
                    ...
                    np.clip(0, 960, bbox[0])
                    np.clip(0, 960, bbox[2])
                    np.clip(0, 960, bbox[1])
                    np.clip(0, 960, bbox[3])
